...
| 5 | Polish | ~~Language-aware CSV filename/header, quiz-email subject line~~ — both already delivered in Phase 3 (see note above). Remaining: final review of any other English-only-sounding strings (dialog titles, About text) | Low-risk, deferrable if time-constrained |
...
| 1.1 (impl.) | 2026-08-24 | Phases 1–4 implemented and verified: data-layer schema/migration (Phase 1), UI re-pointed to new schema (Phase 2), active-language state + toggle scoping every tab incl. CSV/email labeling (Phase 3), Spanish special-character input widget + Unicode NFC normalization (Phase 4). 22/22 automated tests plus three headless Xvfb smoke test scripts all pass. Phase 5 (final string-review polish) and Phase 6 (end-to-end regression pass) remain open. |
